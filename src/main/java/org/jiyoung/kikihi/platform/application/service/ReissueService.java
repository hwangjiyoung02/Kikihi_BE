...
import org.jiyoung.kikihi.platform.adapter.out.redis.token.RefreshTokenRedisHash;
...
//        }
        ...
        refreshRedisRepository.deleteByRefresh(refresh);
    ...
    private void saveRefreshToken(String username, String refresh, Long expiredMs) {
            RefreshTokenRedisHash entity = RefreshTokenRedisHash.builder()
                .refresh(refresh)
                .username(username)
                .expiration(new Date(System.currentTimeMillis() + expiredMs).toString())
                .build();
        refreshRedisRepository.save(entity);
    }
    ...
    private void addRefreshEntity(String username,String refresh,Long expiredMs){
        ...
        refreshTokenRedisHash.setUsername(username);
        refreshTokenRedisHash.setRefresh(refresh);
        refreshTokenRedisHash.setExpiration(date.toString());

        refreshRedisRepository.save(refreshTokenRedisHash);
